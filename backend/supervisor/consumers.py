...
class PongConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        self.username = None
        self.keep_open = True  # Flag to keep the connection open
        self.init_game_state()  # Initialize game state upon connection
    ...
    async def ensure_connection_open(self):
        while self.keep_open:
            ...
            try:
                ...
                logger.error(f'Error keeping connection open: {str(e)}')
                break
...
class PongConsumer(AsyncWebsocketConsumer):
    ...
    def padel_colider(self):
        ...
        if (self.ball["x"] <= 15 and self.player2["y"] <= self.ball["y"] <= self.player2["y"] + paddle_height) or (self.ball["x"] >= 625 and self.player1["y"] <= self.ball["y"] <= self.player1["y"] + paddle_height):
            player = self.player2 if self.ball["x"] <= 15 else self.player1
            relative_intercept = (player["y"] + center_paddle_offset) - self.ball["y"]
            normalized_relative_intercept = relative_intercept / center_paddle_offset
            bounce_angle = normalized_relative_intercept * max_angle
            self.ball["vx"] = abs(self.ball["vx"]) * cos(radians(bounce_angle)) * speed_buff * (-1 if self.ball["x"] > 320 else 1)
            self.ball["vy"] = abs(self.ball["vx"]) * sin(radians(bounce_angle)) * (-1 if normalized_relative_intercept < 0 else 1) * speed_buff
            self.ball["x"] = 15 if self.ball["x"] <= 15 else 625  # Réinitialiser la position de la balle pour éviter le glissement
